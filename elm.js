...
Elm.ChargeShooting = Elm.ChargeShooting || {};
Elm.ChargeShooting.make = function (_elm) {
   "use strict";
   _elm.ChargeShooting = _elm.ChargeShooting || {};
   if (_elm.ChargeShooting.values)
   return _elm.ChargeShooting.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   _P = _N.Ports.make(_elm),
   $moduleName = "ChargeShooting",
   ...
   var inRange = F2(function (limit,
   x) {
      return _U.cmp(0,
      x) < 1 && _U.cmp(x,limit) < 1;
   });
   var moveObject = function (o) {
      return _U.replace([["x"
                         ,o.x + o.vx]
                        ,["y",o.y + o.vy]],
      o);
   };
   var generateCoin = F2(function (x,
   y) {
      return {_: {}
             ,size: 5
             ,vx: 0
   ...
   var genRandomFloat = F3(function (low,
   high,
   s) {
      return A2($Random.generate,
      A2($Random.$float,low,high),
      s);
   });
   ...
   l) {
      return function () {
         var f = F2(function (l,h) {
            return $Basics.fst(A3(genRandomFloat,
            l,
            h,
   ...
   var calcMgn = function (g) {
      return _U.cmp(100,
      g) < 1 ? 10 : (g / 10 | 0) + 1;
   };
   var chargePlayer = F2(function (i,
   p) {
   ...
   _v6) {
      return function () {
         return function () {
            var newComboGauge = function () {
               var t = _v6.comboGauge - 1;
               return _U.cmp(t,0) < 0 ? 0 : t;
            }();
            ...
            newEnemies = $._0,
            generatedCoins = $._1;
            var newCoins = function () {
               var canDraw = _U.cmp(_v6.player.size,
               40) > 0;
               var ratio = 60 - (_v6.player.size - 60);
               var vx$ = function (c) {
                  return canDraw ? (_v6.player.x - c.x) / ratio : 0;
               };
               var vy$ = function (c) {
                  return canDraw ? (_v6.player.y - c.y) / ratio : 0;
               };
               var f = function (c) {
            ...
            var killedEnemyNum = $List.length(_v6.enemies) - $List.length(newEnemies);
            ...
            return _U.replace([["player"
                              ...
                              ,["frame",_v6.frame + 1]
                              ,["isBomb"
                               ,_U.cmp(0,killedEnemyNum) < 0]
                              ,["comboGauge",newComboGauge]],
            _v6);
         }();
      }();
   });
   var isGameOver = function (_v12) {
      return function () {
         return _U.replace([["isGameOver"
                            ,$Basics.not(_v12.player.isLive)]],
         _v12);
      }();
   };
   var soundReset = function (g) {
      return _U.replace([["isBomb"
                         ,false]
                        ,["isGet",false]],
      g);
   };
   ...
   _v14) {
      return function () {
         return function () {
            var isHit = F2(function (o,
            o$) {
               return _U.cmp(Math.pow(o.x - o$.x,
               2) + Math.pow(o.y - o$.y,2),
               Math.pow(o.size + o$.size,
               2)) < 0;
            });
            var isPlayerHitted = A2($List.any,
            isHit(_v14.player),
            _v14.enemies);
            var p = _U.replace([["isLive"
                                ,_v14.player.isLive && $Basics.not(isPlayerHitted)]],
            _v14.player);
            var $ = A3($List.foldr,
            F2(function (b,_v16) {
               return function () {
            ...
            function (e) {
               return A2($List.any,
               isHit(e),
               _v14.bullets) ? _U.replace([["hp"
                                           ,e.hp - 1]],
               e) : e;
            },
            _v14.enemies);
            var newCoins = A2($List.filter,
            function ($) {
               return $Basics.not(isHit(_v14.player)($));
            },
            _v14.coins);
            var gotCoins = $List.length(_v14.coins) - $List.length(newCoins);
            var newScore = _v14.score + gotCoins * calcMgn(_v14.comboGauge);
            var newComboGauge = function () {
               var t = _v14.comboGauge + gotCoins * 10;
               return _U.cmp(gaugeMax,
               t) < 0 ? gaugeMax : t;
            }();
   ...
   var height = 480;
   var moveEnemies = function (es) {
      return function () {
         var updateVY = function (e) {
            return _U.replace([["vx"
                               ,$Basics.cos($Basics.pi * (e.y * 2 / height))]],
            e);
         };
         var move = function ($) {
            return updateVY(moveObject($));
         };
   ...
   _v20) {
      return function () {
         return function () {
            var getX = F2(function (_v22,
            _v23) {
               return function () {
                  switch (_v23.ctor)
                  {case "_Tuple2":
                     return function () {
                          return A3(genRandomFloat,
                          0,
                          width,
                          _v23._1);
                       }();}
                  _U.badCase($moduleName,
                  "on line 202, column 21 to 45");
               }();
            });
            var seed = $Random.initialSeed($Basics.round(_v20.time));
            var xs = $List.tail($List.map($Basics.fst)(A3($List.scanl,
            getX,
            {ctor: "_Tuple2"
            ,_0: 0
            ,_1: seed},
   ...
   var movePlayer = F2(function (i,
   p) {
      return function () {
         var trunc = F2(function (limit,
         x) {
            return _U.cmp(x,
            0) < 0 ? 0 : _U.cmp(limit,
            x) < 0 ? limit : x;
         });
         var nx = trunc(width)($Basics.toFloat($Basics.fst(i.pos)));
         var ny = trunc(height)($Basics.toFloat($Basics.snd(i.pos)));
      ...
      return function () {
         var moveBullet = function (b) {
            return function () {
               ...
               var isReflectY = A3(isReflect,
               height,
               b.y,
               b.vy);
               var nextVY = isReflectY ? 0 - b.vy : b.vy;
               var isReflectX = A3(isReflect,
               width,
               b.x,
               b.vx);
               var nextVX = isReflectX ? 0 - b.vx : b.vx;
               var nextLevel = b.level - (isReflectY || isReflectX ? 1 : 0);
               var b$ = _U.replace([["vx"
                                    ,nextVX]
                                   ,["vy",nextVY]
                                   ,["level",nextLevel]],
   ...
   var moveObjects = F2(function (i,
   _v28) {
      return function () {
         return _U.replace([["player"
                            ,A2(movePlayer,i,_v28.player)]
                           ,["bullets"
                            ,moveBullets(_v28.bullets)]
                           ,["enemies"
                            ,moveEnemies(_v28.enemies)]
                           ,["coins"
                            ,A2($List.map,
                            moveObject,
                            _v28.coins)]],
   ...
   var background = function (gauge) {
      return function () {
         var range = gaugeMax - 100;
         var s1 = range / 3;
         var s2 = range / 3;
         var s3 = range / 4;
         var g = $Basics.toFloat(gauge);
         var a1 = _U.cmp(100,
         g) < 0 && _U.cmp(g,
         100 + s1) < 1 ? (g - 100) / s1 : 0;
         var a2 = _U.cmp(100 + s1,
         g) < 0 && _U.cmp(g,
         100 + s2) < 1 ? (g - 100 - s1) / s2 : 0;
         var a3 = _U.cmp(100 + s2,
         g) < 0 && _U.cmp(g,
         gaugeMax) < 1 ? (g - 100 - s2) / s3 : 0;
         return $Graphics$Element.layers(_L.fromArray([A3($Graphics$Element.image,
                                                      width,
                                                      height,
                                                      "./img/base.png")
                                                      ,$Graphics$Element.opacity(a1)(A3($Graphics$Element.image,
                                                      width,
                                                      height,
                                                      "./img/b1.png"))
                                                      ,$Graphics$Element.opacity(a2)(A3($Graphics$Element.image,
                                                      width,
                                                      height,
                                                      "./img/b2.png"))
                                                      ,$Graphics$Element.opacity(a3)(A3($Graphics$Element.image,
                                                      width,
                                                      height,
                                                      "./img/b3.png"))]));
      }();
      ...
      return function () {
         var toForm = function (_v30) {
            return function () {
               return A2(moveForm,
               _v30.x,
               _v30.y)($Graphics$Collage.outlined($Graphics$Collage.solid(_v30.color))(A2($Graphics$Collage.ngon,
               6,
               _v30.size)));
            }();
         };
   ...
   var playerForm = function (player) {
      return A2(moveForm,
      player.x,
      player.y)($Graphics$Collage.filled($Color.black)($Graphics$Collage.circle(player.size)));
   };
      ...
      return function () {
         var toForm = function (_v32) {
            return function () {
               return A2(moveForm,
               _v32.x,
               _v32.y)($Graphics$Collage.filled(_v32.color)(A2($Graphics$Collage.ngon,
               6,
               $Basics.toFloat(_v32.level * 3))));
            }();
         };
         return $Graphics$Collage.group(A2($List.map,
         toForm,
   ...
   var enemiesForm = function (es) {
      return function () {
         var toForm = function (_v34) {
            return function () {
               return A2(moveForm,
               _v34.x,
               _v34.y)($Graphics$Collage.filled($Color.blue)($Graphics$Collage.square(_v34.size)));
            }();
         };
         return $Graphics$Collage.group(A2($List.map,
         toForm,
   ...
   var coinsForm = function (cs) {
      return function () {
         var toForm = function (_v36) {
            return function () {
               return A2(moveForm,
               _v36.x,
               _v36.y)($Graphics$Collage.filled(A3($Color.rgb,
               255,
               210,
               90))($Graphics$Collage.circle(_v36.size)));
            }();
         };
         return $Graphics$Collage.group(A2($List.map,
         toForm,
   ...
   var gaugeForm = function (gauge) {
      return function () {
         var mgn = calcMgn(gauge);
         var width = _U.cmp(gauge,
         100) > 0 ? 200 : $Basics.toFloat(gauge) * 2;
         return $Graphics$Collage.group(_L.fromArray([A2(moveForm,
                                                     100,
                                                     10)($Graphics$Collage.toForm($Text.plainText(A2($Basics._op["++"],
                                                     "x ",
                                                     $Basics.toString(mgn)))))
                                                     ,A2(moveForm,
                                                     120 + width / 2,
                                                     10)($Graphics$Collage.filled($Color.yellow)(A2($Graphics$Collage.rect,
                                                     width,
                                                     12)))]));
      }();
   };
   var display = function (_v38) {
      return function () {
         return _v38.isGameOver ? gameOverScene(_v38) : $Graphics$Element.layers(_L.fromArray([background(_v38.comboGauge)
                                                                                              ,A3($Graphics$Collage.collage,
                                                                                              width,
                                                                                              height,
                                                                                              _L.fromArray([effectsForm(_v38.effects)
                                                                                                           ,playerForm(_v38.player)
                                                                                                           ,coinsForm(_v38.coins)
                                                                                                           ,bulletsForm(_v38.bullets)
                                                                                                           ,enemiesForm(_v38.enemies)
                                                                                                           ,gaugeForm(_v38.comboGauge)]))
                                                                                              ,$Text.plainText(A2($Basics._op["++"],
                                                                                              "score:",
                                                                                              $Basics.toString(_v38.score)))]));
      }();
   };
   var fps = 30;
   var input = $Signal.sampleOn($Time.fps(fps))(A2($Signal._op["~"],
   A2($Signal._op["~"],
   A2($Signal._op["~"],
   A2($Signal._op["<~"],
   Input,
   ...
   var stepPlayGame = function (i) {
      return function ($) {
         return update(i)(generateObject(i)(moveObjects(i)(collisionObject(i)(isGameOver($)))));
      };
   };
   ...
   display,
   game);
   var jsPlayMusic = _P.portOut("jsPlayMusic",
   ...
   game));
   var jsPlayBombSound = _P.portOut("jsPlayBombSound",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   A2($Signal._op["<~"],
   ...
   game));
   var jsPlayGetSound = _P.portOut("jsPlayGetSound",
   _P.outgoingSignal(function (v) {
      return v;
   }),
   A2($Signal._op["<~"],
   ...
   game));
   _elm.ChargeShooting.values = {_op: _op
                                ,fps: fps
                                ,width: width
                                ,height: height
                                ,gaugeMax: gaugeMax
